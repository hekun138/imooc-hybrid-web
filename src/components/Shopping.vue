<template>
  <!--
    ...
    3、可以在购物车中，通过+、-按钮来增加或减少商品的购买数量，当商品数量为1的时候，不可减少
    4、加入购物车时，如果该商品已经存在于购物车中，那么购物车中的该商品数量自动+1
    5、购物车中会展示已选的商品总价格和已选商品的总数量，当选中商品或选中商品的数量发生变化时，展示的总价格和总数量也会发生对应的变化
  -->
  <!--
    从商品详情中进入购物车页面
    1、首先进入到 main.vue
...
export default {
  ...
  methods: {
    ...
    computeGoodsTotal: function () {
      // 先去初始化 totalData 的数据，让是否全选默认为 true，当存在商品没有被选中的时候，把 isAll 变为 false
      this.totalData = {
        // 是否全选
        isAll: true,
        // 总价格
