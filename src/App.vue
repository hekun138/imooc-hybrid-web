<template>
  <div id="app">
    <!--
      ...
      在后退页面的时候：原页面（商品列表）执行一个从屏幕中展示的位置，从左到右移动出屏幕的动画
      新页面（首页）执行一个从屏幕左侧，从左到右移动到屏幕中的动画
    -->
    <!--
      页面过渡效果实现逻辑
      1、创建动画的css
      2、判断当前是需要执行跳转动画（进入新页面）还是后退动画（后退动画）
        1、监听路由对象 $route
        2、让页面在跳转的时候，携带包含跳转标记的 params，通过对 params 中跳转标记的判断，来判断当前是否为跳转动画
